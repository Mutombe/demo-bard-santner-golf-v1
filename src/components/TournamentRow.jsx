...
export default function TournamentRow({ round, index, isNext = false }) {
  ...
  return (
    <Link
      to="/register"
      className={`group grid grid-cols-12 items-center gap-3 sm:gap-4 py-5 sm:py-6 px-4 sm:px-6 border-t transition ${
    ...
    >
      ...
      <div className="col-span-2 sm:col-span-1 flex items-center gap-1.5 sm:gap-2">
        <svg width="12" height="16" viewBox="0 0 12 16" aria-hidden="true" className="shrink-0 hidden sm:block">
        ...
        <div className="scoreboard-num text-xl sm:text-3xl text-steel-300 tabular-nums">
      ...
      <div className="col-span-3 sm:col-span-2">
        <div className={`scoreboard-num text-3xl sm:text-5xl ${statusColor} tabular-nums leading-none`}>
          <CountUp to={parseInt(round.day, 10)} pad={2} duration={1000} />
        </div>
        <div className="label-xs text-steel-500 mt-1.5">{round.monthShort} 2025</div>
      </div>
      ...
      <div className="col-span-5 sm:col-span-6">
        ...
        <h3 className="font-display text-base sm:text-2xl uppercase text-navy-900 leading-[1.1]">
          {round.title}
        </h3>
        <p className="text-xs sm:text-sm text-steel-600 mt-1.5 hidden sm:block">{round.blurb}</p>
      </div>

      {/* CTA arrow */}
      <div className="col-span-2 sm:col-span-3 flex justify-end">
        {round.status !== 'past' ? (
          <div className="inline-flex items-center gap-2 bg-navy-900 text-white px-3 sm:px-5 py-2.5 sm:py-3 label-xs font-bold group-hover:bg-orange-500 transition">
